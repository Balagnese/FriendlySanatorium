    ...
    class ClientCreate(ArgsAggregator):
        def __init__(self):
            ...
            self.add_argument('is_child').with_field(fields.Boolean(default=False, required=True))\
                .with_parser(type=bool, default=False)
            ...
            self.add_argument('gender')\
                ...
                .with_parser(type=ClientProfileDto.gender, choices=gender_strs)

            self.add_argument('time_arrival').with_field(fields.DateTime(required=True))\
                .with_parser(type=datetime.datetime)

            self.add_argument('time_departure').with_field(fields.DateTime(required=True)) \
                .with_parser(type=datetime.datetime)

            self.add_argument('diet_id').with_field(fields.Integer(required=True)).with_parser(type=int)
...
class DietDto:
    ...
    class DietCreate(ArgsAggregator):
        def __init__(self):
            ...
            self.add_argument('name').with_field(fields.String(required=True)).with_parser(type=DietDto.check_diet_name, location='json')
            self.add_argument('description').with_field(fields.String(required=True)).with_parser(location='json')
